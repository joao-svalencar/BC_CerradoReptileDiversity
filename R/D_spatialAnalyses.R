...
long_grid_list <- lapply(split_grid_list, function(df) {
  ...
  data.frame(
    grid_id   = df$grid_id[1],
    order     = names(split_by_order),
    n_records = sapply(split_by_order, nrow),
    richness  = sapply(split_by_order, function(sub_df) length(unique(sub_df$species))),
    stringsAsFactors = FALSE
  )
})

model_data <- do.call(rbind, long_grid_list)
rownames(model_data) <- NULL

head(model_data)

model_data$order <- factor(model_data$order,
                           levels = c("Squamata",
                                      "Testudines",
                                      "Crocodylia"))

# counting and overdispersed data (family poisson)
mod <- glm(richness ~ log(n_records + 1)*order,
                   data = model_data,
                   family = poisson)
